...
static LogicalResult checkImplementationStatus(Operation &op) {
  ...
  llvm::TypeSwitch<Operation &>(op)
      ...
      .Case<omp::TargetEnterDataOp, omp::TargetExitDataOp>([&](auto op) {
        checkDepend(op, result);
        // A 'nowait' motion construct that also uses an iterator modifier needs
        // its runtime-sized offloading arrays to outlive the deferred target
        // task. That is handled separately; reject the combination for now so
        // it is not lowered through the synchronous (stack-allocated) path.
...
convertOmpTargetData(Operation *op, llvm::IRBuilderBase &builder,
  ...
  auto genMapInfoCB = [&](InsertPointTy codeGenIP) -> MapInfosTy & {
    ...
    genMapInfos(builder, moduleTranslation, DL, combinedInfo, mapData,
                targetDirective);
    if (!iterInfos.empty()) {
      // The iterator modifier turns the number of map entries into a runtime
      // value, so the offloading arrays are sized at runtime from
      // info.TotalMapCount (static entries plus the per-iterator trip counts).
    ...
    for (auto [i, iterInfo] : llvm::enumerate(iterInfos)) {
      ...
      if (failed(fillIteratorLoop(
              ...
              [&](llvm::Value *linearIV, omp::YieldOp yield) {
                ...
                // The map yielded by the iterator region has no members
                // (member maps with the iterator modifier are rejected when
                // building iterInfos above) and this translation never emits
                // non-contiguous descriptors, so it always expands to a single
                // contiguous offloading entry.
                assert(iterCombinedInfo.BasePointers.size() == 1 &&
                       iterCombinedInfo.NonContigInfo.Offsets.empty() &&
                       "iterator map must expand to a single contiguous entry");
