...
IteratorLoopNestScope::IteratorLoopNestScope(llvm::IRBuilderBase &Builder,
                                            unsigned Dims,
                                            llvm::ArrayRef<llvm::Value *> LBs,
                                            llvm::ArrayRef<llvm::Value *> UBs,
                                            llvm::ArrayRef<llvm::Value *> Steps)
      : B(Builder), Ctx(Builder.getContext()),
        Dims(Dims), LBs(LBs), UBs(UBs), Steps(Steps) {
    ...
    for (unsigned Dimension = 0; Dimension < Dims; ++Dimension) {
      ...
      IVs[Dimension]->addIncoming(LBs[Dimension], (Dimension == 0) ? PreBB : BodyBBs[Dimension - 1]);

      llvm::Value *Cond = B.CreateICmpULE(IVs[Dimension], UBs[Dimension], "omp.it.cmp");
      ...
      llvm::Value *Next = B.CreateAdd(IVs[Dimension], Steps[Dimension], "omp.it.next");
